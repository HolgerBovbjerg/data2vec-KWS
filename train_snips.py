...
from utils.dataset_snips import get_loader
...
def training_pipeline(config):
    ...
    with open(config["train_labels_file"], "r") as f:
        train_labels = f.read().rstrip().split("\n")
    
    with open(config["val_list_file"], "r") as f:
        val_list = f.read().rstrip().split("\n")
    with open(config["val_labels_file"], "r") as f:
        val_labels = f.read().rstrip().split("\n")

    trainloader = get_loader(train_list, train_labels, config, train=True)
    valloader = get_loader(val_list, val_labels, config, train=False)
    ...
    with open(config["test_labels_file"], "r") as f:
        test_labels = f.read().rstrip().split("\n")

    testloader = get_loader(test_list, test_labels, config, train=False)
